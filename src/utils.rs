...
use crate::grammers_session::Session;
use crate::grammers_session::types::DcOption;
use std::{error, fmt};
...
fn export_impl(dc_option: &DcOption) -> Result<String, postcard::Error>
{
...
pub fn export<S>(session: &S) -> Result<String, postcard::Error>
where
    S: Session + Sized
{
    ...
    Ok(export_impl(&dc_option)?)
}

#[cfg(feature = "grammers-session")]
#[doc = include_str!("../docs/export.md")]
pub fn export<S>(session: &S) -> Result<String, Box<dyn error::Error + Send + Sync + 'static>>
where
    S: Session + Sized,
...
pub async fn restore<T, S>(session: &S, value: T) -> Result<(), Box<dyn error::Error + Send + Sync>>
where
    T: AsRef<[u8]>,
	S: Session + Sized,
{
    ...
    cfg_select! {
        feature = "grammers-session-0.9.0" => {
            session.set_home_dc_id(dc_option.id).await;
            session.set_dc_option(&dc_option).await;
        }
        feature = "grammers-session" => {
            let _ = session.set_home_dc_id(dc_option.id).await;
            let _ = session.set_dc_option(&dc_option).await;
        }
    }
...
mod tests {
    ...
    use crate::grammers_session::storages::MemorySession;
    ...
    #[tokio::test]
    async fn save_and_restore() -> Result<(), Box<dyn error::Error + Send + Sync + 'static>> {
        let dc_option = DcOption {
            id: 1234,
        ...
        let restored_dc_option = {
            ...
            let dc_option = session.dc_option(dc_option.id).unwrap();
            cfg_select! {
                feature = "grammers-session-0.9.0" => dc_option,
                feature = "grammers-session" => dc_option.unwrap(),
            }
        };
